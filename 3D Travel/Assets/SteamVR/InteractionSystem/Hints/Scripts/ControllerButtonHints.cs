...
//
// Purpose: Displays text and btnTouchpad hints on the controllers
//
//=============================================================================
	...
	public class ControllerButtonHints : MonoBehaviour
	{
		...
		void OnRenderModelLoaded( SteamVR_RenderModel renderModel, bool succeess )
		{
			...
			if ( renderModel == this.renderModel )
			{
				...
				//Get the btnTouchpad mask for each component of the render model
				using ( var holder = new SteamVR_RenderModel.RenderModelInterfaceHolder() )
				{
					...
					if ( renderModels != null )
					{
						...
						//Uncomment to show the btnTouchpad mask for each component of the render model
		...
		private void CreateAndAddButtonInfo( EVRButtonId buttonID )
		{
			...
			//Get the local transform for the btnTouchpad
		...
		private void ComputeTextEndTransforms()
		{
			...
			//to make the text hints fan out appropriately based on the btnTouchpad's position on the controller.
